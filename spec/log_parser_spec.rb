...
RSpec.describe LogParser do
  ...
  describe '#parse_most_visits' do
    ...
    it 'returns a hash, each value being an array for each row in ascending order' do
      expected_output = { '/help_page/1' =>
                          ['126.318.035.038', '929.398.951.889', '722.247.931.582'],\
  ...
  describe '#descending_order' do
    ...
    it 'returns an  array in descending order for all domains' do
      expected_output = [['/help_page/1', 3],\
                         ['/about', 1],\
                         ['/index', 1],\
                         ['/about/2', 1],\
                         ['/home', 1],\
                         ['/contact', 1]]
      expect(log_parser.descending_order).to eq(expected_output)
    end
  end
  describe '#format_message' do
    ...
    it 'returns a formatted message' do
      expected_output = '/help_page/1 3 visits, /about 1 visit, /index 1 visit, /about/2 1 visit, /home 1 visit, '\
                        '/contact 1 visit'
      expect(log_parser.format_message).to eq(expected_output)
    end
  end
end
